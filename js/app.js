...
const App = (() => {
  ...
  let state = {
    ...
    usMacroMeta: null,
    originalMacroProc: null, // snapshot of FRED values for restore
    isRealtime: false,
    realtimeTimer: null,
    charts: {
      powerRadar: null,
  ...
  async function init() {
    console.log('App: Initializing Sentinel Dashboard...');
    _bindEvents();
    await _loadData();
  }
  ...
  function _bindEvents() {
    ...
    const btnReal = document.getElementById('btn-mode-realtime');
    
    if(btnHist && btnReal) {
        btnHist.addEventListener('click', () => _setMode(false));
        btnReal.addEventListener('click', () => _setMode(true));
    }
  ...
  async function _loadData() {
    ...
    try {
      ...
      state.originalMacroProc = { ...data.usMacroProcessed }; // Snapshot for FRED restore
      
      _populateSimulator(); // Fill auto-fetched data into input fields
      
      if (data.usMacroProcessed._usingFallback) {
        _setStatus('stale', `⚠️ Fallback Data (API blocked) — ${new Date().toLocaleTimeString()}`);
      } else {
  ...
  function _render() {
    ...
    // Determine data to use (Historical vs Realtime Override)
    let displayMacro = state.usMacroProc;
    if (state.isRealtime && countryCode === 'US') {
      displayMacro = RealtimeEngine.getOverrideData(state.usMacroProc);
      // Sync simulator inputs with live realtime values
      _syncSimulatorWithRealtime(displayMacro);
    }

    const quadrantInfo = MacroEngine.determineQuadrant(displayMacro, state.usMacroMeta);
    const cycleInfo = DebtCycleEngine.determineStage(displayMacro, state.usMacroMeta);
    ...
    // Apply Real-Time visual indicators
    _applyRealtimeStyles(state.isRealtime);
  }

  function _setMode(isRealtime) {
    state.isRealtime = isRealtime;
    
    // UI Update
    document.getElementById('btn-mode-historical').classList.toggle('active', !isRealtime);
    document.getElementById('btn-mode-realtime').classList.toggle('active', isRealtime);
    ...
    if (isRealtime) {
        ...
        if (state.originalMacroProc) {
            state.usMacroProc = { ...state.originalMacroProc };
            _populateSimulator();
        }
  ...
  function _startRealtimeTimer() {
    ...
    state.realtimeTimer = setInterval(() => {
        _render(); // Re-render to show ticking numbers
    }, 100);
  }

  function _stopRealtimeTimer() {
  ...
  function _applyRealtimeStyles(isActive) {
    const panels = document.querySelectorAll('.glass-panel');
    panels.forEach(p => p.classList.toggle('realtime-glow', isActive));
    
    // Toggle the LIVE badge in the simulator header
    const rtBadge = document.getElementById('sim-rt-badge');
    if (rtBadge) rtBadge.classList.toggle('active', isActive);
  }

  // ── Realtime ↔ Simulator Sync ──

  /** Source mapping for fields that get realtime overrides */
  const REALTIME_SOURCES = {
    inflation: { label: '⚡ Cleveland Fed CPI Nowcast', icon: '🏦' },
    debtGDP:   { label: '⚡ US Debt Clock 即時模擬',   icon: '💰' }
  };

  /**
   * Push realtime override values INTO the simulator input fields,
   * and attach source-annotation badges below each overridden input.
   */
  function _syncSimulatorWithRealtime(realtimeData) {
    for (const [key, meta] of Object.entries(REALTIME_SOURCES)) {
      const input = document.getElementById(`sim-${key}`);
      if (!input || realtimeData[key] == null) continue;

      const decimals = (key === 'debtGDP') ? 1 : 1;
      input.value = Number(realtimeData[key]).toFixed(decimals);
      input.readOnly = true;

      const group = input.closest('.sim-group');
      if (!group) continue;
      ...
      if (!badge) {
        ...
        badge.className = 'rt-source-badge';
  ...
  /**
   * Remove all realtime badge elements and restore editable state.
   */
  function _clearRealtimeBadges() {
    document.querySelectorAll('.rt-source-badge').forEach(el => el.remove());
    document.querySelectorAll('.sim-group.rt-override').forEach(group => {
      group.classList.remove('rt-override');
  ...
  function _setStatus(type, msg) {
    ...
    if(type==='stale')    dot.classList.add('dot--stale');
